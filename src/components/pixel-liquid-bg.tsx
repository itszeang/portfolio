  ...
  update(time: number) {
...

    {
      ...
      const u = (this.externalForce.mesh.material as THREE.RawShaderMaterial)
        .uniforms;
      // Pointer deltas arrive normalised per axis (so a sideways pixel on a
      // wide screen is a smaller number than a vertical one) and advection
      // then stretches the short axis further. Rescale by aspect so the same
      // pointer distance pushes the fluid equally in every direction.
      const fw = this.fboSize.x;
      const fh = this.fboSize.y;
      const short = Math.min(fw, fh);
      u.force.value.set(
        (mouse.diff.x / 2) * mf * (fw / short),
        (mouse.diff.y / 2) * mf * (fh / short),
      );
...
export function PixelLiquidBg({
  ...
  useEffect(() => {
    ...
    const driver = autoDemo && !shouldReduceMotion
    ...
    let raf = 0;
    let running = !document.hidden && !shouldReduceMotion;
