/* eslint-disable react-hooks/rules-of-hooks, @typescript-eslint/no-explicit-any -- vendored Originkit component, kept as shipped by the Originkit CLI */
...
function __OriginkitBase_DitherReveal(props: DitherRevealProps) {
  const {
...
void main() {
    ...
    // Tinted to the site backdrop palette (#8c0f60 -> #ff85b3).
    vec3 ditherDark = vec3(0.549, 0.059, 0.376);
    vec3 ditherLight = vec3(1.0, 0.522, 0.702);
    ...
    // The image is a pre-cut PNG/WebP, so its own alpha is the hand's shape.
    // (Keying out light pixels here used to punch holes through the skin's
    // highlights, which showed up as dark smudges on the black hero.)
    float subjectAlpha = color.a;
    float finalAlpha = mix(subjectAlpha * 0.8, subjectAlpha, revealAmount);
...
const __originkitPresetProps = {
  "image": {
    "src": "blob:https://www.originkit.dev/fb3e0a34-1cef-4a89-9c18-09a57eea3136"
  },
  "dotSize": 7,
  "revealRadius": 98,
  "revealSoftness": 59
...
export default function DitherReveal(props: Record<string, unknown>) {
  return <__OriginkitBase_DitherReveal {...(__originkitPresetProps as Record<string, unknown>)} {...props} />;
}
